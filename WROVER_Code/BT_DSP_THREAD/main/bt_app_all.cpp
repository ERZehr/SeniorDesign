 ...
 * THREAD DECLARATIONS
 ******************************/
static thread_args args_thread1;
static thread_args args_thread2;
static thread_args args_thread3;
static thread_args args_thread4;
static thread_args args_thread5;
static thread_args args_thread6;
static thread_args args_thread7;
static thread_args args_thread8;
static thread_args args_thread9;
static thread_args args_thread10;
static thread_args args_thread11;
static thread_args args_thread12;
...
static void bt_av_hdl_a2d_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_A2D_AUDIO_CFG_EVT: {
        ...
        if (a2d->audio_cfg.mcc.type == ESP_A2D_MCT_SBC) {
            ...
            i2s_std_slot_config_t slot_cfg = I2S_STD_MSB_SLOT_DEFAULT_CONFIG(I2S_DATA_BIT_WIDTH_16BIT, (i2s_slot_mode_t)ch_count);
...
void bt_app_a2d_data_cb(const uint8_t *data, uint32_t len)
{
    ...
    uint8_t* out_media = (uint8_t*)malloc(len);

    // Re-initialize thread arguments
    args_thread1 = (thread_args){.in_media = data, .out_media = out_media, .len = len, .thread_id = 1};
    args_thread2 = (thread_args){.in_media = data, .out_media = out_media, .len = len, .thread_id = 2};
    args_thread3 = (thread_args){.in_media = data, .out_media = out_media, .len = len, .thread_id = 3};
    args_thread4 = (thread_args){.in_media = data, .out_media = out_media, .len = len, .thread_id = 4};
    args_thread5 = (thread_args){.in_media = data, .out_media = out_media, .len = len, .thread_id = 5};
    args_thread6 = (thread_args){.in_media = data, .out_media = out_media, .len = len, .thread_id = 6};
    args_thread7 = (thread_args){.in_media = data, .out_media = out_media, .len = len, .thread_id = 7};
    args_thread8 = (thread_args){.in_media = data, .out_media = out_media, .len = len, .thread_id = 8};
    args_thread9 = (thread_args){.in_media = data, .out_media = out_media, .len = len, .thread_id = 9};
    args_thread10 = (thread_args){.in_media = data, .out_media = out_media, .len = len, .thread_id = 10};
    args_thread11 = (thread_args){.in_media = data, .out_media = out_media, .len = len, .thread_id = 11};
    args_thread12 = (thread_args){.in_media = data, .out_media = out_media, .len = len, .thread_id = 12};

    // Initialize and deploy each thread
    esp_pthread_cfg_t cfg = esp_create_default_pthread_config();
    cfg.stack_size = (4 * 1024);
    cfg.inherit_cfg = true;
    esp_pthread_set_cfg(&cfg);

    if(pthread_create(&thread1, NULL, &bt_media_biquad_bilinear_filter, (void *)&args_thread1) != 0) {
        ESP_LOGW(BT_AV_TAG, "DSP Failed! (Thread 1)");
        write_ringbuf(data, len, "I2S");
    }
    else if(pthread_create(&thread2, NULL, &bt_media_biquad_bilinear_filter, (void *)&args_thread2) != 0) {
        ESP_LOGW(BT_AV_TAG, "DSP Failed! (Thread 2)");
        write_ringbuf(data, len, "I2S");
    }
    else if(pthread_create(&thread3, NULL, &bt_media_biquad_bilinear_filter, (void *)&args_thread3) != 0) {
        ESP_LOGW(BT_AV_TAG, "DSP Failed! (Thread 3)");
        write_ringbuf(data, len, "I2S");
    }
    else if(pthread_create(&thread4, NULL, &bt_media_biquad_bilinear_filter, (void *)&args_thread4) != 0) {
        ESP_LOGW(BT_AV_TAG, "DSP Failed! (Thread 4)");
        write_ringbuf(data, len, "I2S");
    }
    else if(pthread_create(&thread5, NULL, &bt_media_biquad_bilinear_filter, (void *)&args_thread5) != 0) {
        ESP_LOGW(BT_AV_TAG, "DSP Failed! (Thread 5)");
        write_ringbuf(data, len, "I2S");
    }
    else if(pthread_create(&thread6, NULL, &bt_media_biquad_bilinear_filter, (void *)&args_thread6) != 0) {
        ESP_LOGW(BT_AV_TAG, "DSP Failed! (Thread 6)");
        write_ringbuf(data, len, "I2S");
    }
    else if(pthread_create(&thread7, NULL, &bt_media_biquad_bilinear_filter, (void *)&args_thread7) != 0) {
        ESP_LOGW(BT_AV_TAG, "DSP Failed! (Thread 7)");
        write_ringbuf(data, len, "I2S");
    }
    else if(pthread_create(&thread8, NULL, &bt_media_biquad_bilinear_filter, (void *)&args_thread8) != 0) {
        ESP_LOGW(BT_AV_TAG, "DSP Failed! (Thread 8)");
        write_ringbuf(data, len, "I2S");
    }
    else if(pthread_create(&thread9, NULL, &bt_media_biquad_bilinear_filter, (void *)&args_thread9) != 0) {
        ESP_LOGW(BT_AV_TAG, "DSP Failed! (Thread 9)");
        write_ringbuf(data, len, "I2S");
    }
    else if(pthread_create(&thread10, NULL, &bt_media_biquad_bilinear_filter, (void *)&args_thread10) != 0) {
        ESP_LOGW(BT_AV_TAG, "DSP Failed! (Thread 10)");
        write_ringbuf(data, len, "I2S");
    }
    else if(pthread_create(&thread11, NULL, &bt_media_biquad_bilinear_filter, (void *)&args_thread11) != 0) {
        ESP_LOGW(BT_AV_TAG, "DSP Failed! (Thread 11)");
        write_ringbuf(data, len, "I2S");
    }
    else if(pthread_create(&thread12, NULL, &bt_media_biquad_bilinear_filter, (void *)&args_thread12) != 0) {
        ESP_LOGW(BT_AV_TAG, "DSP Failed! (Thread 12)");
        write_ringbuf(data, len, "I2S");
    }
    else {
        write_ringbuf(out_media, len, "I2S");
    }
    free(out_media);
