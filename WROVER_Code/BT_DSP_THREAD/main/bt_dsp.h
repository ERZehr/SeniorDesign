 ...
 * @param [out] out_media  byte array out output media data
 * @param [in] thread_id   identifier of the currently-running thread
 * 
 * @return  true if malloc'd and filtered correctly - false otherwise
 */
bool bt_media_biquad_bilinear_filter(const uint8_t *media, uint32_t len, uint8_t* out, int thread_id);
